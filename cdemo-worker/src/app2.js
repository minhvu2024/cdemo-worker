...
class BINLookup{
  ...
  setupEventListeners(){
    ...
    safe('searchBtn','click',()=>this.search());
    safe('clearBtn','click',()=>this.clearFilters());
    safe('copyBinsBtn','click',()=>this.copyBinsList());
    ...
    ['normalizeInput','dupInput','importInput'].forEach(id=>{
      ...
      if(el){
        el.addEventListener('paste',(e)=>{setTimeout(()=>{el.scrollLeft=0},50);});
        const valueProp=Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype,'value');
        if(valueProp){
          const originalSet=valueProp.set;
          Object.defineProperty(el,'value',{
            set:function(val){originalSet.call(this,val);setTimeout(()=>{this.scrollLeft=0},50);},
            get:valueProp.get,
            configurable:true
          });
        }
  ...
  getSearchParams(){
    ...
    if(!params.limit) params.limit=parseInt(document.getElementById('limitInput').value)||50;
  ...
  async search(){
      ...
      if(d.success){
        ...
        if(params.bin && params.bin.includes(',')){
          ...
          this.currentResults.sort((a,b)=>{
             const ia=inputOrder[a.bin]!==undefined?inputOrder[a.bin]:999999;
             const ib=inputOrder[b.bin]!==undefined?inputOrder[b.bin]:999999;
             return ia-ib;
          });
        }
        this.displayResults();
    ...
    }catch(e){this.showError('Search failed')}finally{btn.disabled=false;btn.innerHTML=html}
  }
...
'<div class="glass p-4 rounded-xl hover-lift"><h3 class="font-bold mb-3 flex items-center gap-2 text-gray-900"><i class="fas fa-list text-[#4A8BFF]"></i>Top 10 Types</h3><div class="space-y-2">'+(topTypes.length>0?topTypes.map(t=>'<div class="flex items-center justify-between"><span class="font-semibold text-gray-900">'+(t.type||'UNKNOWN')+'</span><span class="text-sm text-gray-600">'+(t.count||0).toLocaleString()+'</span></div>').join(''):'<div class="text-gray-600 text-center">No data</div>')+'</div></div>'+
'</div>'+
...
'</div>';}}catch(e){content.innerHTML='<div class="text-center text-red-400">Failed to load dashboard</div>';}}
  ...
  async searchBinsForExport(){
    ...
    try{
      ...
      }else{this.showError(d.error||'Search failed');}
    }catch(e){this.showError('Search failed: '+e.message);}finally{btn.disabled=false;btn.innerHTML=html}
  }
  ...
  sortResults(type, col){
    ...
    let list;
    if(type==='search') list=this.currentResults;
    else if(type==='export') list=this.cardExporter.bins;
    else return;
    ...
    if(type==='search') this.displayResults();
    else this.displayBinResults(list);
  }
